# build_mingw64.ps1 - 64-bit release build
# Usage: powershell -NoProfile -ExecutionPolicy Bypass -File script\build_mingw64.ps1
# Output: dist\<datetime>  (DRUPPC.exe + Qt runtime + resource/, nothing packed into exe)

$ErrorActionPreference = "Stop"

$QtRoot     = "D:\Qt5\Qt5.14.2\5.14.2\mingw73_64"
$ToolsRoot  = "D:\Qt5\Qt5.14.2\Tools\mingw730_64"
$ProjectDir = Split-Path -Parent $PSScriptRoot
# Intermediate artifacts live in %TEMP%, never in the workspace; removed after build.
$BuildDir   = Join-Path $env:TEMP "DRUPPC_build_mingw64"
...
# Qt/MinGW runtime dlls
$Dlls = @("Qt5Core.dll", "Qt5Gui.dll", "Qt5Widgets.dll", "Qt5SerialPort.dll", "Qt5Xml.dll", "libgcc_s_seh-1.dll", "libstdc++-6.dll", "libwinpthread-1.dll")
foreach ($d in $Dlls) {
